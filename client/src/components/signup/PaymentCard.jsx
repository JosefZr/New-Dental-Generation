...
const stripePromise = loadStripe('pk_test_51Q6FSwRsgnrIRIXHVv98PFAvJYYVK9gElLXl8fV16Xquu3PHduekcmJ182SsDLAcgNRjOSKzxAJmTZQO8nUpo720001usG5YNY');
export default function PaymentCard({ cardData, dentist, store, lab }) {
    ...
    const handleCheckout = async () => {
        ...
        const response = await axios.post("http://localhost:3000/api/v1/payment/create-checkout-session", {
          items: [{  name:name, amount, quantity: 1 }], // replace "Service Name" as appropriate
        });
        ...
        <div className="w-full flex justify-center lg:pl-9 2xl:pl-0">
            ...
            <div className="2xl:h-auto 2xl:pb-0 2xl:overflow-y-visible w-full 2xl:w-auto 2xl:inline-flex 2xl:flex-row 2xl:rounded-lg 2xl:border-none gap-3 mt-4 2xl:mt-6 mx-auto max-w-[950px]">
                {cardData.map((card, index) => (
                    <div 
                        key={index} 
                        style={selectedCardIndex ===index ?{ position: "relative", border:"1px solid var(--gold)" }:{position:"relative"}}
                        onClick={() =>handleCardSelect(index, card.amount, card.name) }
                        className={` rounded-lg py-4 px-5 text-white transition-all flex flex-col justify-between items-start  cursor-pointer border-2 border-transparent hover:border-my-gold hover:border-opacity-30 mb-[14px] 2xl:h-[393.5px]  bg-[#1B1E26] ${selectedCardIndex === index ? "border border-my-gold  " : ""}`}
                    >
                        {/* Top Section with Free Trial or Title */}
                        {card.dentistFreeTrial && dentist ? (
                            <div className="flex items-center justify-between w-full">
                                <div className="text-[18px] font-semibold">6 days free trial</div>
                                <button
                                    className={`absolute bottom-3 right-5 2xl:hidden transition-transform group-hover:translate-y-[-1px] group-hover:text-my-gold ${
                                        selectedCardIndex === index ? "rotate-90 text-my-gold" : ""
                                    }`}
                                >
                        ...
                        ) : card.storeLabFreeTrial && (lab || store) ? (
                            <div className="flex items-center justify-between w-full">
                                <div className="text-[18px] font-semibold">40 days free trial</div>
                                <button
                                    className={`absolute bottom-3 right-5 2xl:hidden transition-transform group-hover:translate-y-[-1px] group-hover:text-my-gold ${
                                        selectedCardIndex === index ? "rotate-90 text-my-gold" : ""
                                    }`}
                ...
                <>
                    ...
                    <div className='flex items-center'>
                        ...
                        <div className='text-[16px] text-[#FFFFFF80] ml-1'>/ monthly</div>
                    </div>
                    </div>
                    ...
                    <button 
                        className={`hidden 2xl:block h-[54px] rounded-md bg-[#393C45]  font-bold font-league w-full hover:shadow-xl hover:translate-y-[-1px]  duration-200 transform hover:bg-my-gold hover:text-[#06060C] transition-all bg-primary-gradient text-[#06060C] shadow-glowy translate-y-[-1px] ${selectedCardIndex===index ?"shadow-[-5px_0px_70px_-15px_#ffc100]":""}`}
                        style={selectedCardIndex ===index ? {backgroundColor:"var(--gold)",color:"var(--black)!important" }:{}}
                    >
                    <div className={`text-xl  text-my-white-gray `} style={selectedCardIndex ===index ?{color:"var(--black)"}:{}}>{selectedCardIndex ===index ? "SELECTED": "CHOOSE PLAN"}</div>
                    </button> 
                    <button
                ...
                <>
                    <span className="hidden w-full 2xl:flex items-center justify-center mb-[24px] uppercase">
                        <div 
                            ...
                            style={selectedCardIndex === index ?{backgroundColor:"var(--gold)", color:"var(--black)"}:{}}    
                        >
                    ...
                    <div className='flex items-center'>
                        ...
                        <div className='text-[16px] text-[#FFFFFF80] ml-1'>/ 4 months</div>
                    </div>
                    {card.threeMonthsButton && (
                ...
                <>
                    ...
                    <div className='flex items-center'>
                        ...
                        <div className='text-[16px] text-[#FFFFFF80] ml-1'>/ yearly</div>
                    </div>
                    {card.yearButton && (
                        <span className="2xl:hidden flex items-center justify-center uppercase">
                        <div className="inline-flex mx-auto rounded-full items-center h-[32px] bg-[#343740] py-2 px-4 text-[12px] font-extrabold tracking-[0.1em] text-[#1B1E26] bg-my-small-gold">
                    ...
                    <button 
                        className={`hidden 2xl:block h-[54px] rounded-md bg-[#393C45]  font-bold font-league w-full hover:shadow-xl hover:translate-y-[-1px]  duration-200 transform hover:bg-my-gold hover:text-[#06060C] transition-all bg-primary-gradient text-[#06060C] shadow-glowy translate-y-[-1px] ${selectedCardIndex===index ?"shadow-[-5px_0px_70px_-15px_#ffc100]":""}`}
                        style={selectedCardIndex ===index ? {backgroundColor:"var(--gold)",color:"black" }:{}}
                    >
                    <div className={`text-xl  text-my-white-gray `} style={selectedCardIndex ===index ?{color:"var(--black)"}:{}}>{selectedCardIndex ===index ? "SELECTED": "CHOOSE PLAN"}</div>
                    </button> 
                    <button
