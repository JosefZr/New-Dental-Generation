...
import { LeftSignup, PaymentCard, PersonalInformationForm, SelectPaymentMethod } from '@/components/signup';
    ...
    return [
      {
        ...
        month: transformDescriptionToTable("Access to all TRW Campuses. Daily live broadcasts. Daily course updates."),
      },
      {
        name:"quarterly",
        amount:55,
        ...
        threeMonths:transformDescriptionToTable("All of Cadet. Daily coin bonus. Power level boost")
      },
      {
        name:"yearly",
        amount:150,
        ...
        year:transformDescriptionToTable("Maximum daily coin bonus. Big power level boost. Exclusive chats and lessons.")
      },
      {
...
export default function Signup() {
  ...
  const handleImageChange = (e) => {
    ...
    const handleSubmit = async(e) => {
      ...
      try{
        const result = await fetch ("https://api.cloudinary.com/v1_1/dmbqu5vfl/image/upload",{
          method:"POST",
          body:formDataObject,
        }).then(r=> r.json())
...
<section className="lg:flex-row w-full font-sans flex flex-col h-[100vh] lg:overflow-y-scroll relative">
    <LeftSignup/>
    <main
      className="lg:w-[50vw] flex flex-col pr-[22px] pl-[23px] h-full"
    >
        <div className='hidden lg:block '>
          <div className='lg:flex-row lg:h-[114px] lg:text-left flex flex-col mt-[25px] text-primary-content text-center lg:w-full'>
            <img className='trw-logo mx-auto lg:mx-0 lg:mr-3 max-w-[114px] max-h-[114px]' src="	https://www.jointherealworld.com/checkout/_next/static/media/trw-knight-globe.c69d0d31.svg" alt="logo" />
            <div>
              <p className=' lg:text-[27px] font-black text-[25px] mx-auto mt-[18px] undefined'>JOIN THE REAL WORLD</p>
              <p className=' lg:text-[24px] text-[21px] mx-auto mt-[11px]'>ESCAPE THE MATRIX</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-7 mt-8 mb-8">
            ...
            <div>
              ...
              <PaymentCard  cardData={data} dentist={formData.role==="dentist"} lab={formData.role==="lab"} store={formData.role==="store"}/>
            </div>
            {/* Payment card info */}
        ...
        <div className="col-span-6 sm:flex sm:items-center sm:gap-4">
          ...
          <p className="mt-4 text-sm text-gray-500 sm:mt-0">
            ...
            <Link to="/signup" className="text-gray-700 underline">Log in</Link>.
          </p>
        </div>
    </form>
